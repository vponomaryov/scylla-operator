    ...
    spec:
      ...
      - name: cassandra-stress
        ...
        command:
          - "bash"
          - "-c"
          - "cassandra-stress {d.cmd} -node {d.host} | tee /{d.name}-{}.log"
        resources:
          limits:
            cpu: {d.cpu}
            memory: {d.memory}
...
def parse():
    ...
    parser.add_argument('--namespace', default='scylla', help='namespace of the cassandra-stress jobs - defaults to "scylla"')
    ...
    parser.add_argument('--cpu', default='1', help='number of cpus that will be used for each job - defaults to 1')
...
def create_job_list(args):
    ...
    for i in range(args.num_jobs):
        manifests.append(template.format(i, i, d=args))
    return manifests
